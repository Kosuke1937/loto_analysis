...
def support_hist(agents,C,topn=500):
 weighted=np.zeros(44,float);raw=np.zeros(44,int);agent_hits=np.zeros(44,int)
 for name,idx in agents.items():
  ...
  for x in seen:agent_hits[x]+=1
 return weighted,raw,agent_hits
...
def diverse(order,combs,n=10):
 sel=[];tc=Counter();pc=Counter();nc=Counter()
 for i in order:
  row=combs[int(i)];trs=list(itertools.combinations(row,3));pas=list(itertools.combinations(row,2))
...
def main():
 rows=p.fetch_history(); rows=[r for r in rows if r[0]<=2133];draws=np.asarray([r[1] for r in rows],np.int16);bonus=np.asarray([r[2] for r in rows],np.int16);t=len(draws)
 ...
 weighted,raw,ah=support_hist(agents,C,500)
 recent20=set(map(int,draws[-20:].ravel()));eligible=[x for x in range(1,44) if x in recent20 and x not in EXCLUDE]
 eligible.sort(key=lambda x:(weighted[x],raw[x],-x),reverse=True);pool=eligible[:22]
 hist=[{'num':x,'weighted_support':float(weighted[x]),'raw_top500_occurrences':int(raw[x]),'agent_coverage':int(ah[x]),'eligible':x in eligible,'in_pool22':x in pool} for x in sorted(range(1,44),key=lambda x:(weighted[x],raw[x],-x),reverse=True)]
 # shape temporal conditions
 ...
 prev=set(map(int,draws[-1]));
 combs=[];meta=[]
 for c in itertools.combinations(sorted(pool),6):
  sh=band(c)
  if sh20[sh]>0:continue
  mode='21-50_once' if sh21_50[sh]==1 else ('new50' if sh50[sh]==0 else None)
  if mode is None:continue
  if len(set(c)&prev)>1:continue
  combs.append(c);meta.append((sh,mode))
 if not combs:raise SystemExit('no candidates')
 X,fx=direct_features(combs,qcuts)
 # build current Stat500 weights and score direct candidates
 ...
 order=np.argsort(score)[::-1];sel=diverse(order,combs,10)
 tickets=[]
 for i in sel:
  sh,mode=meta[i];tickets.append({'nums':list(combs[i]),'sum':sum(combs[i]),'shape':list(sh),'shape_long_freq':shall[sh]/t,'shape_21_50_count':sh21_50[sh],'shape_50_count':sh50[sh],'mode':mode,'prev_overlap':len(set(combs[i])&prev),'stat_score':float(stat[i]),'core5':float(dc5[i]),'core4':float(dc4[i]),'committee_score':float(score[i]),'rebuild_rank':int(np.where(order==i)[0][0])+1})
 out={'target_draw':2134,'method':'usual multi-agent Top500 number support -> top22 eligible -> temporal-shape rebuild -> Committee rerank','formula':'Z(Stat500)+0.20Z(5core)+0.15Z(4core), z reference=fixed 60k pre-draw sample','exclusions':sorted(EXCLUDE),'recent20_only_numbers':True,'pool22':pool,'histogram':hist,'rebuild_candidate_count':len(combs),'tickets':tickets}
 (OUT/'loto6_hist_rebuild_2134.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(out,ensure_ascii=False,indent=2))
