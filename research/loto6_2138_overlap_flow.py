...
from loto6_2137_cluster_flow_committee import (
    ...
    cluster_audit, committee_scores, local_cluster
)
...
def calibrate(rows, window=500):
    # Walk-forward: for each target draw, build the four flow sets only from prior draws.
    # Compare winner hits by condition-overlap multiplicity, correcting for class size.
    ...
    return {
        ...
        'winner_overlap_composition_top':[
            {'composition_0_1_2_3plus':list(k),'draws':v}
            for k,v in draw_comps.most_common(12)
        ],
...
def choose_plan(trans):
    # Convert observed S1 transition rates to ten slots, but guarantee at least one slot per state.
    rates=trans['rates']
    raw={s:rates[s]*10 for s in rates}
    alloc={s:max(1,int(math.floor(raw[s]))) for s in rates}
    while sum(alloc.values())<10:
        s=max(rates,key=lambda x:(raw[x]-alloc[x],rates[x]))
        alloc[s]+=1
    while sum(alloc.values())>10:
        cand=[s for s in rates if alloc[s]>1]
        s=min(cand,key=lambda x:(raw[x]-alloc[x],rates[x]))
        alloc[s]-=1
    order=sorted(alloc,key=lambda s:(-rates[s],s))
    plan=[]
...
def main():
    ...
    sm=shape_meta(hist)
    cal=calibrate(rows,500)
    trans=sum_transition(rows,'S1',500)
    alloc,state_plan=choose_plan(trans)

    # Per-ticket target overlap composition. Main idea: 0-1 overlap numbers dominate,
    # 2-overlap remains normal, 3+ is only a small rescue signal.
    # Slots are diversified rather than hard-coded to one exact composition.
    comp_plan=[
        (2,3,1,0),(2,2,2,0),(3,2,1,0),(1,3,2,0),(2,3,1,0),(2,2,2,0),
        (2,2,1,1),(1,3,1,1),(3,2,1,0),(2,2,2,0)
    ]
    layer_plan=['A','B','C','A','B','C','A','B','D','A']
    ...
    for pos,st in enumerate(state_plan):
        ...
        for relax in range(4):
            for x in candidates:
                ...
                # At most one 3+ overlap number per line, and only two portfolio lines should use them.
                if len(x['high_overlap_numbers'])>1: continue
                if x['high_overlap_numbers'] and sum(highuse.values())>=2: continue
                comp=x['overlap_comp']
                compdist=sum(abs(comp[i]-target[i]) for i in range(4))
                ...
                k=(
                    compdist,
                    x['cluster_viol'],
                    0 if x['layer']==layer_plan[pos] else 1,
                    shapeuse[x['shape']],
        ...
        assert best is not None,(pos,st,target)
    ...
    out={
        ...
        'calibration':cal,
        'sum_transition':trans,
        'sum_state_allocation':alloc,
        'portfolio':[]
    }
    for i,(x,s) in enumerate(zip(selected,scores),1):
        ...
        z['cluster_detail']={k:v for k,v in z['cluster_detail'].items()}
        z.update(s);z['slot']=i
        out['portfolio'].append(z)
    p=OUT/'loto6_2138_overlap_flow.json';p.write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
