...
def freq_strata_mask(C,inc,npref,t,prevbo):
    ...
    low=order[:14];mid=order[14:28];high=order[28:]
    cl=inc[:,low].sum(1);cm=inc[:,mid].sum(1);ch=inc[:,high].sum(1)
    return (cl>=1)&(cm>=1)&(ch>=1)&(cl<=4)&(cm<=4)&(ch<=4)

def greedy(ids,C,pairids,tripids,layers,shapes,mode='pair',ptotal=None,pmed=0.0,n=10):
    ids=np.asarray(ids,dtype=np.int64)
    if len(ids)==0:return []
    sel=[];chosen=np.zeros(len(C),bool);usedn=np.zeros(44,bool);usedp=np.zeros(44*44,bool);usedt=np.zeros(44*44*44,bool)
    nc=np.zeros(44,np.int16);pc=np.zeros(44*44,np.int16);tc=np.zeros(44*44*44,np.int16);lc=Counter();sc=Counter()
    for strict,layer_cap,shape_cap in [(True,True,True),(False,True,True),(False,False,True),(False,False,False)]:
        while len(sel)<n:
            av=ids[~chosen[ids]]
            if len(av)==0:break
            ok=np.ones(len(av),bool)
            if layer_cap:ok &= np.asarray([lc[str(x)]<QUOTA[str(x)] for x in layers[av]])
            if shape_cap:ok &= np.asarray([sc[int(x)]<2 for x in shapes[av]])
            if strict:
                ok &= (nc[C[av]].max(1)<4)&(pc[pairids[av]].max(1)<2)&(tc[tripids[av]].max(1)<1)
            av=av[ok]
            if len(av)==0:break
            ...
            elif mode=='graph' and ptotal is not None:keys=(av,load,np.abs(ptotal[av]-pmed),-newn,-newt,-newp)
            else:keys=(av,load,-newn,-newt,-newp)
            i=int(av[int(np.lexsort(keys)[0])]);chosen[i]=True;sel.append(i);row=C[i];pp=pairids[i];tt=tripids[i]
    ...
    for i in ids:
        i=int(i)
        if not chosen[i]:sel.append(i);chosen[i]=True
        if len(sel)>=n:break
    return sel[:n]
...
def adaptive_vote_pool(routes,base_ids,profile_ids,C,prevbo,min_candidates=250):
    vote=Counter()
    for arr in routes.values():
        for i in arr:
            for x in map(int,C[i]):vote[x]+=1
    nums=[x for x in range(1,44) if x!=prevbo];nums.sort(key=lambda x:(-vote[x],x))
    rank=np.full(44,99,np.int16)
    for j,x in enumerate(nums,1):rank[x]=j
    maxrank=rank[C].max(1)
    chosen_ids=None;eff=None;pool=None
    for k in range(22,33):
        ids=profile_ids[maxrank[profile_ids]<=k]
        if len(ids)<min_candidates:ids=base_ids[maxrank[base_ids]<=k]
        if len(ids)>=min_candidates or k==32:
            chosen_ids=ids;eff=k;pool=nums[:k];break
    return chosen_ids,eff,pool
...
def summary(rr):
    if not rr:return {}
    keys=['best','winner_union','portfolio_numbers','winner_pairs','winner_triples','portfolio_pairs','portfolio_triples'];out={'n':len(rr),'mean_tickets':float(np.mean([x['n_tickets'] for x in rr]))}
    ...
    return out
def paired(a,b,key):return {'gain':sum(y[key]>x[key] for x,y in zip(a,b)),'loss':sum(y[key]<x[key] for x,y in zip(a,b)),'tie':sum(y[key]==x[key] for x,y in zip(a,b))}
...
def main():
    ...
    for draw in range(EVAL_START,END+1):
        ...
        profmask=profile_mask(draws,t,rawsum,rng,odd,con);profile_ids=np.where(base&profmask)[0]
        fmask=freq_strata_mask(C,inc,npref,t,prevbo);freq_ids=np.where(base&fmask)[0]
        pairc=ppref[t]-ppref[max(0,t-300)];ptotal=pairc.reshape(-1)[pairids].sum(1);hp=pair_total_for_rows(draws[max(0,t-200):t],pairc);plo,phi=np.quantile(hp,[.10,.90]);pmed=float(np.median(hp));graph_ids=np.where(base&profmask&(ptotal>=plo)&(ptotal<=phi))[0]
        routes={'structure':greedy(base_ids,C,pairids,tripids,layers,shapes,'pair'),'profile':greedy(profile_ids if len(profile_ids)>=20 else base_ids,C,pairids,tripids,layers,shapes,'pair'),'graph':greedy(graph_ids if len(graph_ids)>=20 else profile_ids,C,pairids,tripids,layers,shapes,'graph',ptotal,pmed),'freq':greedy(freq_ids if len(freq_ids)>=20 else base_ids,C,pairids,tripids,layers,shapes,'number')}
        cons=round_robin(routes,10);avid,ek,pool=adaptive_vote_pool(routes,base_ids,profile_ids,C,prevbo);avsel=greedy(avid,C,pairids,tripids,layers,shapes,'pair')
        win=tuple(map(int,draws[t]));sels={**routes,'consensus':cons,'adaptive_vote':avsel};rec={'draw':draw,'winner':list(win),'previous_bonus':prevbo,'base_count':len(base_ids),'profile_count':len(profile_ids),'graph_count':len(graph_ids),'freq_count':len(freq_ids),'adaptive_pool_size':ek,'adaptive_pool':pool};effp.append(ek)
        for name,sel in sels.items():
            mm=met(win,sel,C);R[name].append(mm);rec[name]=mm
            if draw in (2134,2135):rec[name+'_tickets']=[list(map(int,C[i])) for i in sel]
        details.append(rec)
        if draw%25==0:print(draw,flush=True)
    allsum={k:summary(v) for k,v in R.items()};early={k:summary(v[:154]) for k,v in R.items()};late={k:summary(v[154:]) for k,v in R.items()}
    out={'definition':'Score-light/no-Committee final-selection study on fixed deterministic 60k sample. All agents use only pre-draw data. Base gate: temporal allowed band shape, previous main-number overlap <=1, previous bonus excluded. No hard sum>previous-sum rule. Structure maximizes pair/triple/number novelty. Profile adds rolling winner central structural ranges. Graph adds rolling pair-cooccurrence range and uses pair-profile only as a lexicographic tie-break. Freq balances low/mid/high recent-100 frequency strata. Consensus round-robins the four routes. Adaptive_vote forms the smallest 22-32 number pool with sufficient candidates from sub-agent ticket votes, then assembles exactly 10 by pair coverage.','evaluation':[EVAL_START,END],'n_draws':len(details),'summary':allsum,'early_1828_1981':early,'late_1982_2135':late,'adaptive_pool':{'mean':float(np.mean(effp)),'median':float(np.median(effp)),'distribution':{str(k):sum(x==k for x in effp) for k in range(22,33)}},'paired_vs_structure':{name:{k:paired(R['structure'],R[name],k) for k in ('best','winner_union','winner_pairs','winner_triples','d3','d4')} for name in R if name!='structure'},'draw2134':next((x for x in details if x['draw']==2134),None),'draw2135':next((x for x in details if x['draw']==2135),None)}
